---
tagline: "Pre-Sales Solution Architect"
---
...
- Gewährleistete die Betriebsbereitschaft des Datenbank- und Analytics-Stacks durch Konfiguration eines ETL-Prozesses in einen Cassandra Feature Store.
...
- Entwickelte und containerisierte (Docker) Backend-Dienste, leitete Systemdesign-Reviews zur Optimierung der Datenpersistenz und transaktionaler Workflows über MySQL und MongoDB hinweg.
- Realisierte neue datengetriebene Features und APIs, etablierte robuste Dokumentationsstandards für skalierbare Informationsverwaltungssysteme.
...
- Leitete Systemdesign-Reviews und Code-Architektur-Reviews für kritische datenbankgestützte Dienste und mentorierte Junior-Entwickler in Bezug auf skalierbare Datenpersistenzmuster.
...
- Entwickelte eine robuste, wiederholbare Dokumenten-Ingestion-Pipeline mit Hatchet (extract → chunk → embed → summarize → index) mit Timeout-Konfiguration pro Schritt und absturzsicheren Wiederholungsmechanismen.
...
- Bereitgestellt cloud-nativ auf k3d mit cert-manager TLS und ExternalDNS, wobei alle Inferenzen lokal über zwei llama.cpp-Server hinter einer OpenAI-kompatiblen API gehalten wurden.
...
- **Migrierte den Session-Status und die Messaging-Infrastruktur der Plattform** von Dapr zu nativem **NATS JetStream (Streaming)** und **NATS KV (Session-Status)** mit direkten gRPC-Verbindungen zur Optimierung der Transaktionslatenz.
- **Integrierte mehrere Drittanbieter** über einen Adapter-/Anti-Korruptionsvertrag mit **CUE-basierter Schema-Zuordnung**, um die strukturelle Datenintegrität über heterogene Quellen hinweg sicherzustellen.
- **Bereitgestellt Go-Mikrodienste** unter Verwendung von **Kubernetes**, **kustomize**, **Skaffold** und **Cilium**, um robustes, isoliertes Routing und hochverfügbare Betriebsabläufe zu gewährleisten.
