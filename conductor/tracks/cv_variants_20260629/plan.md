...
- [ ] Task: Create the `data/cv-variants/` directory.
- [ ] Task: Update `engine/shared/config.py` (or similar configuration file) to include a mapping dictionary that links the taxonomy cluster keys (e.g., `ml-ai`) to their respective static markdown filenames (`ml-ai.md`).
