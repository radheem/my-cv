...
_CV_SYSTEM = (
    "You write a tailored, one-page CV body in GitHub-flavored Markdown. Treat this CV as "
    "a high-signal \"landing page\" designed to encourage the viewer to visit the portfolio link "
    "for full details. You MUST be ruthless with your word count and focus purely on technical scale and impact.\n\n"
    "Rules:\n"
    "- TRUTH ONLY: use only facts from the provided master CV. Never invent "
    "roles, employers, dates, skills, or metrics.\n"
    "- RUTHLESS BREVITY (EXPERIENCE): Reorder and reword experience bullets to foreground what the job needs. "
    "You MUST limit recent roles (last 2 jobs) to exactly 2 punchy, single-sentence bullet points. Limit older "
    "roles to exactly 1 bullet point. Add a final line 'Tech: ...' listing only relevant keywords.\n"
    "- RUTHLESS BREVITY (PROJECTS): Feature EXACTLY the provided top-2 projects (no more, no fewer). "
    "In ## Projects (which you can also title ## Selected Work), format each project with an H3 heading exactly "
    "like '### <exact project name>'. Underneath each heading, you MUST write exactly 2 concise, punchy bullet points "
    "(using '- ') highlighting the technical impact. Use ONLY facts from that project's highlights and the "
    "master CV; never invent.\n"
    "- RUTHLESS BREVITY (SKILLS): You MUST consolidate the skills list from the master CV into exactly 3 tight, "
    "high-signal categories/rows:\n"
    "  1. *Languages & Core*: Languages (spoken), Programming Languages, and core paradigms.\n"
    ...
    "- Education entries use EXACTLY this two-line format — institution name as '### Org' "
    "(H3, no degree in the heading), then an italic line '*Degree · Dates*'. Do NOT add coursework, "
    "GPA, focus areas, or bullet points under Education unless they appear verbatim in the master CV.\n"
    ...
    "- Keep it tight (STRICTLY one page). Output Markdown only — no code fences, no "
    "commentary."
)

_COVER_SYSTEM = (
    "You write the BODY of a tailored cover letter in GitHub-flavored Markdown. It must "
    "strictly follow a two-question H2-based structure, ~150-250 words total, optimizing "
    "for a punchy, highly scannable reading experience. Rules:\n"
    "- TRUTH ONLY: no invented experience, skills, metrics, or dates.\n"
    "- STRUCTURE: You must output exactly two Markdown H2 headings. If generating in English: "
    "`## 1. Why <Actual Company Name>?` and `## 2. Why me?` (where `<Actual Company Name>` is "
    "replaced by the actual company name). If generating in German: `## 1. Warum <Actual Company Name>?` and "
    "`## 2. Warum ich?`. You MUST never leave literal brackets, placeholders, or \"[Company]\" in your headings; "
    "always use the actual company name provided.\n"
    ...
    "- 1. WHY COMPANY?: do NOT open with 'I am excited to apply' or any variant of that cliche. "
    "Open with ONE genuine, specific idea that connects your motivation to THIS company's mission or problem, "
    "then name the role. This section MUST naturally establish your personal career timing and fit. Match the TONE "
    "of this example (style only): \"A good monitoring system tells you what is wrong before a customer ever notices — "
    "building that quiet kind of reliability is the part of operations I enjoy most.\"\n"
    "- 2. WHY ME? (Highlight Bullets): Do NOT write dense paragraphs. Instead, provide exactly 3 punchy bullet points "
    "(using '- ') that highlight the absolute strongest matching technical achievements from the CV. Provide concrete, "
    "specific proof with named systems and real numbers.\n"
    "- LOGISTICS: After the 3 bullets in the \"Why me?\" section, seamlessly weave your availability, relocation "
    "willingness, and work-authorization facts (if provided) into a single, natural sign-off sentence.\n"
