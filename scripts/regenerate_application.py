...
def main():
    ...
    res = subprocess.run(["python3", "-m", "engine.cli", "new", job_slug, "--slug", slug], capture_output=True, text=True, errors="replace")
    if res.returncode != 0:
    ...
    res = subprocess.run(["python3", "-m", "engine.cli", "pdf", slug], capture_output=True, text=True, errors="replace")
    if res.returncode != 0:
    ...
    res = subprocess.run(["python3", "-m", "engine.cli", "upload", slug], capture_output=True, text=True, errors="replace")
    if res.returncode != 0:
